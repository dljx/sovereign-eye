...
| `nav:broker:v1` | `/api/dd/nav-broker` POST (broker_sync.py) | `/api/nav-history` | real daily broker NAV/flows/income/trades, USD, broker-scoped: `{IBKR: {navs:[{date,nav}], flows:[{date,amount}], income:[{date,amount,type,ticker}], trades:[{date,ticker,realized,qty,side}], twr, updated}, Tiger: {...}}`. `/api/nav-history` serves it (common-span sum + VWRA/SPY + flow-adjusted TWR in `perf` incl. `perf.byBroker`; `realized` = 12m realized-P&L summary; per-ticker `income`) instead of snapshots. `trades` needs the IBKR Flex Trades section. |
| `alerts:state:v1` | `/api/dd/alert-state` POST (watch_triggers.py) | same GET | trigger-engine dedup memory: `{"<TICKER>|<rule>": "<ISO ts>"}`, one entry per fired (ticker,rule); 60-day GC + 500-key cap. |
| `thesis:daryl` | `/api/dd/thesis` POST (thesis_check.py, daily portfolio run) + PUT (dashboard manual edit) | same GET (Portfolio tab dots) | per-holding thesis registry + adherence checks: `{TICKER: {thesis, key_swing, set_at, source: system\|manual, status: INTACT\|STRAINED\|BROKEN\|UNKNOWN, adherence, reason, checked_at}}`. Thesis TEXT is a stable anchor — POST seeds once, never rewrites; only PUT (manual) or reset changes it. |
