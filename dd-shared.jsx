      ...
      {open && (
        <div className="ev-grid">
          ...
          <div>
            ...
            <EvRow k="NTM / LT PEG" v={ratios.fwd_peg != null || ratios.peg_lt != null ? `${n(ratios.fwd_peg, 2) ?? '·'} / ${n(ratios.peg_lt, 2) ?? '·'}${ratios.eps_cagr_fwd_years ? ` (${ratios.eps_cagr_fwd_years}y)` : ''}` : null} title="NTM = Fwd P/E ÷ next-year EPS growth (1-year PEG — a rebound year off a low base compresses it without implying durable cheapness). LT = Fwd P/E ÷ a longer-horizon consensus EPS growth rate, known basis — source chain Finviz true-5yr (basis-cross-checked) → FMP (~2yr) → Alpha Vantage (~1-2yr); the (Ny) suffix shows which horizon actually resolved. NTM cheap while LT ordinary means short-horizon-only cheapness — but check BOTH cheap together too: a multi-year earnings recovery (e.g. post-impairment) can depress both readings similarly with no divergence to catch (v4, 2026-07-13)" />
